...
def calculate_metrics(y_true, y_pred):
    ...
    rmse = round(np.sqrt(mean_squared_error(y_true_clean, y_pred_clean)), 4)
...
def main():
    parser = argparse.ArgumentParser(description="Multi-task model evaluation table")
    ...
    # Export full long-format table (includes all Valid_N and MAE; suitable for SI appendix)
    result_df.to_csv(args.output_csv, index=False)
    print(f"Detailed long table saved to: {args.output_csv}")
    
    # Keep only RMSE and R2 for wide-format pivot
    ...
    # Order columns: GIPR -> GLP1R -> GCGR, with Train -> Test within each target
    base_cols = ["Model"]
    ...
    pivot_df_summary = pd.concat([pivot_df,\
                                  pd.DataFrame(data=pivot_df.iloc[:,1:].mean().round(4)).T,\
