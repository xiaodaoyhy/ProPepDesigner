...
from generate_finetune import build_peptide
...
#########################################
# Update pep_sequence_model and pep_smiles_model
# to backbone state by removing long side chains
##########################################
aa_df = pd.read_csv('../../myutils/cut_AA_183.txt')
aa_dict = dict(zip(aa_df['Name'], aa_df['Smiles']))

for r_i in range(len(df_init)):
    seq_ls = eval(df_init.loc[r_i, sequence_model_name])
    smiles_ls = eval(df_init.loc[r_i, pep_model_smiles])
    new_seq_ls = []
    new_smiles_ls = []
    for j,s in enumerate(seq_ls):
        if '*' in s:
            s_temp = s.replace('*', '')
            new_seq_ls.append(s_temp)
            new_smiles_ls.append(aa_dict.get(s_temp, smiles_ls[j]))
        else:
             new_seq_ls.append(s)
             new_smiles_ls.append(smiles_ls[j])
    df_init.loc[r_i, sequence_model_name] = str(new_seq_ls)
    df_init.loc[r_i, pep_model_smiles] = str(new_smiles_ls)
    try:
        df_init.loc[r_i, 'Backbone_smiles'] = build_peptide(amino_acids=new_smiles_ls, topology='Linear', masking_positions=[0])[0]
    except:
        df_init.loc[r_i, 'Backbone_smiles'] = None
        print(f"Failed to assemble peptide for ID={df_init.loc[r_i, ID]}: {seq_ls}")

##########################################


# Drop rows that cannot be processed (e.g., cyclic peptides)
df_process1 = df_init[~df_init['Backbone_smiles'].isna()]
df = df_process1.query(f'{length_name} <= 45').reset_index(drop=True)
print(f'Peptides with length > 45: {len(df_process1)-len(df)}')


summary_data = df.drop_duplicates(subset=['Backbone_smiles']).reset_index(drop=True).loc[:, [ID, 'Backbone_smiles',\
...
summary_data.to_csv(outdir / 'process_summary1_new_removeLongside.csv', index=None)
print(f'All data processed. Final row count: {len(summary_data)}')
summary_data['aa_count'] = summary_data[sequence_model_name].apply(lambda x: Counter(eval(x)))
counts = Counter()
